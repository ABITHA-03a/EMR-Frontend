...
export default function RegisterScreen() {
  ...
  const {
    control,
    handleSubmit,
    watch, // Used to get the value of password for comparison
    formState: { errors },
  } = useForm({
    defaultValues: {
      email: '',
  ...
  const onSubmit = (data) => {
    ...
    Alert.alert('Registration Attempt', `Email: ${data.email}\nPassword: ${data.password}`);
  ...
  const formInputColors = {
    ...
    inputOutlineColor: '#cccccc',
    activeOutlineColor: '#4d5dff',
    labelColor: '#444444',
  };
          ...
          <View style={isTablet ? styles.formContainerTablet : styles.formContainer}>
            ...
            >
              <View style={{ width: '100%', maxWidth: 400, alignItems: 'center' }}>
                <IconButton
                  icon="account-plus" // Changed icon for register
                ...
                <FormInput
                  ...
                  rules={{
                    ...
                    validate: (value) =>
                      value === password || 'Passwords do not match', // Compare with 'password' field
                  }}
                  {...formInputColors}
                  onFocus={handleInputFocus}
                  onBlur={handleInputBlur}
...
lottieAnimation: {
    width: '100%', // Make the animation fill its container
    height: '100%',
    // You might need to adjust aspect ratio or fit if the animation looks stretched
    // aspectRatio: 1, // Uncomment if your animation is square
     resizeMode: 'cover', // Experiment with these if it doesn't look right
  },
  overlay: {
    ...StyleSheet.absoluteFillObject,
